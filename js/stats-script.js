function generateLabels(timeRange, count) {
  switch (timeRange) {
    case 'day': return Array.from({ length: count }, (_, i) => `Day ${i + 1}`);
    case 'week': return Array.from({ length: count }, (_, i) => `Week ${i + 1}`);
    case 'month': return ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'].slice(0, count);
  }
}
